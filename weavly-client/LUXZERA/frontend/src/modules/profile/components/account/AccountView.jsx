...
const AccountView = ({ 
  ...
  // Reusable minimal form field wrapper
  ...
  const inputBase = "w-full h-11 rounded-xl border border-neutral-200 bg-neutral-50/40 hover:bg-white focus:bg-white focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900/10 text-[13.5px] font-medium text-neutral-900 placeholder-neutral-400 outline-none transition-all duration-150 shadow-2xs";
  ...
  return (
    <div className="relative space-y-5">
      ...
      {/* Status Messages */}
      {errorMsg && (
        <div className="px-4 py-3 bg-red-50/80 border border-red-200/80 rounded-xl text-[12.5px] font-medium text-red-700 text-center shadow-2xs">
          {errorMsg}
        </div>
      )}
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSave(fileInputRef.current); }} className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 p-5 sm:p-6 rounded-2xl bg-white border border-neutral-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
          ...
          <div className="flex-1 min-w-0 text-center sm:text-left">
            <div className="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
              <h3 className="text-[17px] font-semibold text-neutral-900 tracking-tight">{fullName}</h3>
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10.5px] font-semibold uppercase tracking-wider bg-neutral-100 text-neutral-700 border border-neutral-200/60">
            ...
            <div className="flex items-center justify-center sm:justify-start gap-2.5 mt-3.5">
              <button 
                ...
                className="inline-flex items-center gap-1.5 text-xs font-medium text-neutral-800 hover:bg-neutral-100/80 transition-all bg-white border border-neutral-200 rounded-lg px-3 py-1.5 shadow-2xs"
              >
        ...
        <div className="p-5 sm:p-6 rounded-2xl bg-white border border-neutral-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-5">
          <div className="flex items-center gap-2 pb-3.5 border-b border-neutral-100">
            <User size={15} strokeWidth={2} className="text-neutral-500" />
            <h4 className="text-[13px] font-semibold text-neutral-900 tracking-tight">
              Personal Details
            </h4>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            <FormField label="First Name" icon={User}>
              <input
                ...
                className={`${inputBase} pl-9.5 pr-4`}
                required
              />
            </FormField>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            <FormField label="Gender">
              <select
                ...
                onChange={onFormChange}
                className={`${inputBase} px-3.5 cursor-pointer appearance-none bg-[url('data:image/svg+xml;charset=UTF-8,%3Csvg%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%20width%3D%2214%22%20height%3D%2214%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%2371717A%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C/polyline%3E%3C/svg%3E')] bg-[length:14px] bg-[right_14px_center] bg-no-repeat pr-9`}
              >
            ...
            <FormField label="Date of Birth" icon={Calendar}>
              <input
                ...
                onChange={onFormChange}
                className={`${inputBase} pl-9.5 pr-4 cursor-pointer`}
              />
            </FormField>
          </div>
        </div>
        ...
        <div className="p-5 sm:p-6 rounded-2xl bg-white border border-neutral-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-5">
          <div className="flex items-center gap-2 pb-3.5 border-b border-neutral-100">
            <Sparkles size={15} strokeWidth={2} className="text-neutral-500" />
            <h4 className="text-[13px] font-semibold text-neutral-900 tracking-tight">
              Contact & Style Bio
            </h4>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            <FormField label="Mobile Number" icon={Phone}>
              <input
                ...
                className={`${inputBase} pl-9.5 pr-4 text-xs font-mono`}
              />
            </FormField>
